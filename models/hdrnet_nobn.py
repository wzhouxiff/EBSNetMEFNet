...
class Guide(nn.Module):
    ...
    '''
    def __init__(self, mode="PointwiseNN", inc=3):
        ...
        if mode == "PointwiseNN":
            ...
            self.conv1 = conv_block(self.inc, 16, kernel_size=1, padding=0, is_BN=False)
...
class Transform(nn.Module):
    ...
    def forward(self, coeff, full_res_input):
        ...
        full_res_input = torch.cat([full_res_input, torch.ones((b,1,h,w), dtype=torch.float32).cuda()], dim=1)
        if self.outc == 1:
...
class HDRNet(nn.Module):
    def __init__(self, inc=3, outc=3, guide_mode='PointwiseNN'):
        ...
        # -----------------------------------------------------------------------
        global_conv_layers = [
            conv_block(64, 64, stride=2, activation=self.activation, is_BN=False),
            conv_block(64, 64, stride=2, activation=self.activation, is_BN=False),
        ]
        self.global_conv = nn.Sequential(*global_conv_layers)

        global_fc_layers = [
            fc(1024, 256, activation=self.activation, is_BN=False),
            fc(256, 128, activation=self.activation, is_BN=False),
            fc(128, 64)
        ]
        ...
        self.guide_func = Guide(inc=self.inc, mode=guide_mode)
