...
class Charbonnier_loss(nn.Module):
    """L1 Charbonnierloss."""
    def __init__(self, epsilon=1e-6, noreduce=False):
        super(Charbonnier_loss, self).__init__()
        self.eps = epsilon ** 2
        self.noreduce = noreduce

    def forward(self, X, Y):
        # batchsize = X.data.shape[0]
        diff = X - Y
        if not self.noreduce:
            loss = torch.mean(torch.sqrt(diff ** 2 + self.eps)) 
        else:
            loss = torch.sqrt(diff ** 2 + self.eps) 
        return loss
...
def C_Loss(output, label, noreduce=False):
    c_loss_func = Charbonnier_loss(epsilon=1e-3, noreduce=noreduce)
